...
@decorator
def evidence(theta, h):
    """Computes P(h | theta) by marginalizing over c."""
    w, mu1, mu2, sigma1, sigma2 = theta

    dist1 = stats.norm(loc=mu1, scale=sigma1)
    dist2 = stats.norm(loc=mu2, scale=sigma2)
    return w * dist1.pdf(h) + (1 - w) * dist2.pdf(h)


def log_likelihood(theta):
    """Computes the log-likelihood over all observed heights for theta."""
    # a product in normal space is a sum in log space, and powers become multiplications
    # so this is a weighted sum
    log_probs = np.log(evidence(theta, height_values))
    return log_probs @ height_weights


def compute_alpha(theta_prime, theta):
    """Compute the threshold used to determine whether to accept the new value of theta."""
    # e^x / e^y = e^(x - y)
    return np.exp(log_likelihood(theta_prime) - log_likelihood(theta))
```
...
Another stab at a[^testing] footnote

[^testing]: *Testing* **formatting** and $math$

And then a link to the previous one [^query] [^testing]

[^query]: <div style="color: red;">testing styling</div>

    - Example one
    - Example two

    $$
    \int_{x \in \mathbb{R}} \int_{y \in \mathbb{R}} e^{x^2 + y^2}\ dy\ dx = \sqrt{\pi}
    $$
